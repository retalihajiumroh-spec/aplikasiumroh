...
export function CommunityEngagement({
  ...
  return (
    ...
    >
      <h2 className="text-lg font-semibold text-emerald-50">Engagement komunitas</h2>
      ...
      <div className="mt-6 grid gap-6 sm:grid-cols-2 sm:items-center">
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-xl border border-emerald-500/15 bg-emerald-950/40 p-3">
            <div className="flex items-center justify-between gap-2">
              <p className="text-[11px] font-semibold uppercase tracking-wide text-emerald-500/60">Lead engagement</p>
              <p className="font-mono text-sm font-semibold text-amber-200/95">{leadPct}%</p>
            </div>
            <p className="mt-1 text-[11px] text-slate-500/90">{leadEngagementPipeline.label}</p>
            <div className="mt-2 h-2 overflow-hidden rounded-full bg-emerald-950/80 ring-1 ring-emerald-500/15">
              <motion.div
                className="h-full rounded-full bg-gradient-to-r from-sky-400/90 to-emerald-400/85"
                initial={{ width: 0 }}
                animate={inView ? { width: `${Math.min(100, leadPct)}%` } : { width: 0 }}
                transition={{ duration: 0.9, delay: 0.15, ease: [0.22, 1, 0.36, 1] }}
              />
            </div>
            <p className="mt-2 text-[11px] text-emerald-500/55">
              <span className="font-mono text-emerald-200/85">{leadEngagementPipeline.activeLeads.toLocaleString("id-ID")}</span> aktif · total{" "}
