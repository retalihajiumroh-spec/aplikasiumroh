...
function BrandBlock() {
  return (
    <Link href="/dashboard" className="flex shrink-0 items-center gap-2.5 rounded-xl py-1 transition hover:opacity-95">
      <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-emerald-400/35 via-emerald-600/25 to-amber-500/25 text-sm font-bold text-emerald-50 ring-1 ring-amber-400/30">
        S
      </span>
      <div className="leading-tight">
        <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-amber-200/85">SA&apos;YA</p>
...
export function Topbar({ user }: { user: ShellUser }) {
  ...
  return (
    <motion.header
      ...
      className="sticky top-0 z-40 border-b border-emerald-500/10 bg-emerald-950/65 px-4 py-3 backdrop-blur-xl sm:px-6 lg:px-8"
    >
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-3 sm:flex-row sm:items-center sm:justify-end sm:gap-3">
          ...
          <div className="relative min-w-0 flex-1 sm:max-w-md lg:max-w-lg">
            ...
            <input
              ...
              className="w-full rounded-2xl border border-emerald-500/15 bg-emerald-950/50 py-2.5 pl-10 pr-4 text-sm text-emerald-50 placeholder:text-slate-500/80 focus:border-amber-400/40 focus:outline-none focus:ring-2 focus:ring-amber-500/20"
            />
          </div>
