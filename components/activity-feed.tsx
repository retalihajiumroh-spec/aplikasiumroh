...
function kindAccent(kind: OverviewActivityKind) {
  switch (kind) {
    case "payment":
      return "from-zinc-500/30 to-zinc-700/15 border-zinc-500/25";
    case "document":
      return "from-sky-400/25 to-zinc-600/10 border-sky-400/20";
    case "booking":
      return "from-zinc-500/30 to-orange-500/15 border-zinc-500/25";
    default:
      return "from-zinc-500/25 to-fuchsia-500/10 border-zinc-500/20";
  }
}
  ...
  return (
    <section className="glass-panel rounded-2xl border border-zinc-600/10 p-5 sm:p-6">
      <div className="border-b border-zinc-600/10 pb-4">
        ...
        <p className="mt-1 text-sm text-zinc-300/50">{description}</p>
      </div>
        ...
        {items.map((item, i) => {
          ...
          return (
            <motion.li
              ...
              className="flex gap-3 rounded-xl border border-zinc-600/10 bg-zinc-950/30 p-3 transition hover:border-zinc-500/25"
            >
              <span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border bg-gradient-to-br ${ring}`}>
                <Icon className="h-[18px] w-[18px] text-zinc-50/95" aria-hidden />
              </span>
              <div className="min-w-0 flex-1">
                <p className="text-sm font-semibold text-zinc-50">{item.title}</p>
                <p className="mt-0.5 text-xs text-zinc-300/55">{item.description}</p>
                <div className="mt-2 flex flex-wrap items-center gap-x-2 gap-y-1 text-[11px] text-zinc-500/60">
                  <span className="font-mono">{item.at}</span>
                  {item.amountIdr !== undefined ? (
                    <span className="font-mono font-semibold text-zinc-300/90">{formatIdrCompact(item.amountIdr)}</span>
                  ) : null}
