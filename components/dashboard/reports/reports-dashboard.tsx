  ...
  return (
    <div className="relative min-h-dvh overflow-x-hidden pb-20 pt-6 sm:pt-10">
      <div
        ...
        style={{
          backgroundImage:
            "radial-gradient(circle at 20% 0%, rgba(52, 211, 153, 0.12), transparent 42%), radial-gradient(circle at 85% 40%, rgba(45, 212, 191, 0.08), transparent 40%)",
        }}
      />
      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        ...
        >
          <div>
            ...
            <h1 className="mt-2 text-3xl font-semibold tracking-tight text-emerald-50 sm:text-4xl">Reports</h1>
        ...
        <motion.section
          ...
          className="mt-8 glass-panel rounded-2xl border border-emerald-500/10 p-4 sm:p-5"
        >
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 className="text-sm font-semibold text-emerald-50">Ekspor laporan</h2>
              <p className="mt-0.5 text-xs text-emerald-200/50">Pilih format unduhan atau simulasi generate berkas.</p>
            </div>
            <div className="flex flex-wrap gap-2">
              ...
              <button
                ...
                className="inline-flex items-center justify-center gap-2 rounded-xl border border-emerald-400/25 bg-emerald-500/12 px-4 py-2.5 text-sm font-medium text-emerald-50 transition hover:border-emerald-400/45 hover:bg-emerald-500/20 disabled:cursor-not-allowed disabled:opacity-45"
              >
          ...
          {lastMessage ? (
            <motion.p
              ...
              animate={{ opacity: 1, y: 0 }}
              className="mt-3 rounded-lg border border-emerald-500/15 bg-emerald-950/40 px-3 py-2 text-xs text-emerald-200/80"
              role="status"
            >
              {lastMessage}
            </motion.p>
        ...
        <div className="mt-6 flex flex-wrap items-baseline justify-between gap-2 border-b border-emerald-500/10 pb-3">
          <h2 className="text-lg font-semibold text-emerald-50">Ringkasan keuangan</h2>
          <p className="font-mono text-xs text-emerald-300/70">{s.periodLabel}</p>
        </div>
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {cards.map((c, i) => (
            <motion.div
              ...
              className="glass-card rounded-2xl border border-emerald-500/10 p-4 sm:p-5"
            >
              <p className="text-[10px] font-semibold uppercase tracking-wider text-emerald-400/65">{c.label}</p>
              <p className="mt-2 font-mono text-xl font-semibold tracking-tight text-emerald-50 sm:text-2xl">{c.value}</p>
              <p className="mt-2 text-xs text-emerald-200/45">{c.hint}</p>
            </motion.div>
          ))}
        </div>
        ...
        >
          <div className="flex flex-col gap-2 border-b border-emerald-500/10 pb-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <h2 className="text-lg font-semibold text-emerald-50">Arus per paket (top)</h2>
              <p className="mt-1 text-sm text-emerald-200/50">Revenue, biaya langsing, margin — untuk lampiran laporan manajemen.</p>
            </div>
            <button
              type="button"
              disabled={exporting !== null}
              onClick={handleCsv}
              className="shrink-0 rounded-xl border border-white/10 bg-white/[0.06] px-3 py-2 text-xs font-medium text-emerald-100 transition hover:bg-white/10 disabled:opacity-45"
            >
              Sertakan di CSV
            </button>
          </div>
          ...
          <div className="mt-4 overflow-x-auto">
            <table className="w-full min-w-[640px] border-collapse text-left text-sm">
              <thead>
                <tr className="border-b border-emerald-500/15 text-[10px] font-semibold uppercase tracking-wider text-emerald-400/70">
              ...
              <tbody>
                {topPackageCashflow.map((row, i) => (
                  ...
                  >
                    <td className="py-3 pr-4 font-medium text-emerald-100/95">{row.packageName}</td>
                    <td className="py-3 pr-4 text-right font-mono text-emerald-200/90">{formatIdr(row.revenueIdr)}</td>
                    <td className="py-3 pr-4 text-right font-mono text-emerald-200/70">{formatIdr(row.costIdr)}</td>
                    <td className="py-3 pr-4 text-right font-mono text-emerald-300/90">{row.marginPct.toFixed(1)}%</td>
                    <td className="py-3 text-right font-mono text-emerald-200/75">{row.pax}</td>
                  </motion.tr>
                ))}
              </tbody>
            </table>
