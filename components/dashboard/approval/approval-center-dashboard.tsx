...
function ApprovalCard({
  ...
  return (
    <motion.article
      ...
      className="rounded-xl border border-zinc-600/12 bg-zinc-950/30 p-4"
    >
      <div className="flex flex-wrap items-start justify-between gap-2">
        <h3 className="text-sm font-semibold leading-snug text-zinc-50 sm:text-base">{item.title}</h3>
        {item.amountIdr != null ? (
          <span className="shrink-0 rounded-lg border border-zinc-500/25 bg-zinc-600/10 px-2 py-1 font-mono text-xs text-zinc-200">
            {formatIdr(item.amountIdr)}
          </span>
        ) : null}
      </div>
      <p className="mt-2 text-xs leading-relaxed text-zinc-300/55 sm:text-sm">{item.summary}</p>
      <div className="mt-3 flex flex-wrap gap-x-3 gap-y-1 text-[11px] text-zinc-500/60">
        <span>{item.submittedBy}</span>
        <span className="font-mono text-zinc-400/70">{item.submittedAtLabel}</span>
      </div>
      <div className="mt-4 flex flex-wrap gap-2">
        <motion.button
          ...
          className="rounded-xl border border-zinc-500/40 bg-gradient-to-r from-zinc-600/25 to-zinc-900/15 px-4 py-2 text-xs font-semibold text-zinc-50 sm:text-sm"
        >
          Setujui
        </motion.button>
        <motion.button
      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        ...
        >
          <div>
            ...
            <h1 className="mt-2 text-3xl font-semibold tracking-tight text-zinc-50 sm:text-4xl">Approval Center</h1>
            <p className="mt-2 max-w-2xl text-sm text-zinc-300/55 sm:text-base">
              Antrian persetujuan untuk <strong className="font-medium text-zinc-200/90">Cabang</strong> dan{" "}
              <strong className="font-medium text-zinc-200/90">Mitra</strong> — tombol Setujui / Tolak tersimpan lokal (demo).
            </p>
          </div>
          <Link
            href="/"
        ...
        <div className="mt-10 grid gap-8 lg:grid-cols-2">
          ...
          >
            <div className="flex items-center justify-between gap-2 border-b border-zinc-600/10 pb-4">
              <h2 className="text-lg font-semibold text-zinc-50">Antrian Cabang</h2>
              <span className="rounded-full border border-zinc-500/25 bg-zinc-600/15 px-2.5 py-1 font-mono text-xs text-zinc-300">
                {pendingCabangCount} menunggu
              </span>
            </div>
            <p className="mt-3 text-sm text-zinc-300/50">Permohonan dari kantor cabang yang memerlukan persetujuan pusat.</p>
            <div className="mt-5 space-y-4">
              <AnimatePresence mode="popLayout">
                {cabangPending.length === 0 ? (
                  <p className="py-8 text-center text-sm text-zinc-300/45">Tidak ada antrian cabang.</p>
                ) : (
          ...
          >
            <div className="flex items-center justify-between gap-2 border-b border-zinc-600/10 pb-4">
              <h2 className="text-lg font-semibold text-zinc-50">Antrian Mitra</h2>
            ...
            <p className="mt-3 text-sm text-zinc-300/50">Registrasi, tier, diskon khusus, dan permintaan mitra.</p>
            <div className="mt-5 space-y-4">
              <AnimatePresence mode="popLayout">
                {mitraPending.length === 0 ? (
                  <p className="py-8 text-center text-sm text-zinc-300/45">Tidak ada antrian mitra.</p>
                ) : (
        ...
        {recentResolved.length > 0 ? (
          ...
          >
            <h2 className="text-sm font-semibold text-zinc-50">Riwayat singkat (sesi ini)</h2>
            <p className="mt-1 text-xs text-zinc-300/45">Maks. 4 entri terakhir setelah Setujui atau Tolak.</p>
            <ul className="mt-4 divide-y divide-zinc-600/10">
              {recentResolved.map((r) => (
                <li key={r.id} className="flex flex-wrap items-center justify-between gap-2 py-3 text-sm">
                  <div className="min-w-0">
                    <p className="truncate font-medium text-zinc-200/90">{r.title}</p>
                    <p className="text-xs text-zinc-500/55">
                  ...
                  <span
                    className={`shrink-0 rounded-md border px-2 py-1 text-[10px] font-bold uppercase tracking-wide ${
                      r.decision === "approved"
                        ? "border-zinc-500/35 bg-zinc-600/15 text-zinc-300"
