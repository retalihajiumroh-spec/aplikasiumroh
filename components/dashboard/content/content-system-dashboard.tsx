  ...
  return (
    <div className="relative min-h-dvh overflow-x-hidden pb-20 pt-6 sm:pt-10">
      <div
        ...
        style={{
          backgroundImage:
            "radial-gradient(circle at 25% 0%, rgba(52, 211, 153, 0.12), transparent 42%), radial-gradient(circle at 80% 40%, rgba(45, 212, 191, 0.1), transparent 40%)",
        }}
      />
      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        ...
        >
          <div>
            ...
            <h1 className="mt-2 text-3xl font-semibold tracking-tight text-emerald-50 sm:text-4xl">Content System</h1>
        ...
        <div className="mt-10 grid gap-8 lg:grid-cols-2 lg:items-start">
          ...
          >
            ...
            <p className="mt-1 text-sm text-emerald-200/55">Link referensi Instagram Reels dan TikTok, plus brief singkat.</p>

            <label className="mt-6 block">
              <span className="text-xs font-semibold uppercase tracking-wider text-emerald-300/60">Link Instagram Reels</span>
              <input
                type="url"
                value={reelsUrl}
            ...
            <label className="mt-5 block">
              <span className="text-xs font-semibold uppercase tracking-wider text-emerald-300/60">Link TikTok</span>
              <input
                type="url"
                value={tiktokUrl}
            ...
            <label className="mt-5 block">
              <span className="text-xs font-semibold uppercase tracking-wider text-emerald-300/60">Brief (opsional)</span>
              <textarea
                value={brief}
            ...
            <motion.button
              ...
              className="mt-6 flex w-full items-center justify-center gap-2 rounded-xl border border-emerald-400/35 bg-gradient-to-r from-emerald-500/25 to-teal-600/20 px-4 py-3 text-sm font-semibold text-emerald-50 shadow-lg shadow-emerald-950/30 disabled:cursor-wait disabled:opacity-70"
            >
              {loading ? (
                <>
            ...
            <p className="mt-3 text-center text-[11px] text-emerald-500/50">Output berikut contoh statis (tidak memanggil API).</p>
          </motion.section>
            ...
            <AnimatePresence mode="wait">
              {!ready ? (
                <motion.div
                  ...
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
                  className="glass-panel flex min-h-[420px] flex-col items-center justify-center rounded-2xl border border-dashed border-emerald-500/20 p-8 text-center"
                >
                  <p className="text-sm font-medium text-emerald-100/90">Pratinjau belum tampil</p>
                  <p className="mt-2 max-w-sm text-xs leading-relaxed text-emerald-200/45">
                    Isi link Reels &amp; TikTok, lalu ketuk <span className="text-emerald-300/80">Generate pratinjau</span> untuk melihat
                ...
                >
                  <section className="glass-panel rounded-2xl p-5 sm:p-6">
                    <div className="flex items-center justify-between gap-2">
                      <h2 className="text-lg font-semibold text-emerald-50">Caption</h2>
                      <CopyTextButton text={preview.caption} />
                    </div>
                    <p className="mt-3 whitespace-pre-line text-sm leading-relaxed text-emerald-100/90">{preview.caption}</p>
                  </section>

                  <section className="glass-panel rounded-2xl p-5 sm:p-6">
                    <div className="flex items-center justify-between gap-2">
                      <h2 className="text-lg font-semibold text-emerald-50">Ad script</h2>
                      <CopyTextButton text={preview.adScript} label="Salin skrip" />
                    </div>
                    <pre className="mt-3 whitespace-pre-wrap font-mono text-xs leading-relaxed text-emerald-100/85">{preview.adScript}</pre>
                  </section>

                  <section className="glass-panel rounded-2xl border border-emerald-400/20 bg-emerald-500/[0.06] p-5 sm:p-6">
                    <div className="flex items-center justify-between gap-2">
                      <h2 className="text-lg font-semibold text-emerald-50">CTA</h2>
                      <CopyTextButton text={preview.cta} label="Salin CTA" />
                    </div>
                    <p className="mt-3 text-sm leading-relaxed text-emerald-50/95">{preview.cta}</p>
                  </section>
                </motion.div>
              )}
            </AnimatePresence>
