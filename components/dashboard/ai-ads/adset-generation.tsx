...
function MediaPreview({ ad }: { ad: AdCreative }) {
  ...
  return (
    <div className={`relative w-full overflow-hidden rounded-xl border border-zinc-600/15 ${aspect}`}>
      ...
      <span className="absolute right-2 top-2 rounded-md bg-zinc-950/60 px-2 py-0.5 text-[9px] font-semibold text-zinc-300/90 backdrop-blur-sm">
        SA&apos;YA
      </span>
    </div>
  );
...
function AdSetColumn({
  ...
  return (
    <motion.div
      ...
      className={`glass-panel rounded-2xl p-5 sm:p-6 ${targeted ? "ring-1 ring-zinc-500/20" : "ring-1 ring-zinc-500/15"}`}
    >
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div>
          <span
            className={`inline-flex rounded-full border px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wide ${
              targeted ? "border-zinc-500/30 bg-zinc-700/15 text-teal-100" : "border-zinc-500/30 bg-zinc-600/12 text-zinc-300"
            }`}
          >
            {targeted ? "Targeted" : "Broad"}
          </span>
          <h3 className="mt-3 text-base font-semibold text-zinc-50">{pack.title}</h3>
          <p className="mt-1 text-xs leading-relaxed text-zinc-300/50">{audienceDisplay}</p>
        </div>
        <span className="rounded-full border border-white/10 bg-white/[0.04] px-2 py-1 font-mono text-[11px] text-zinc-400/70">
          3 ads
        </span>
      </div>
      ...
      <ul className="mt-6 space-y-4">
        {pack.ads.map((ad, i) => (
          ...
          >
            <MediaPreview ad={ad} />
            <div className="p-3.5">
              <p className="text-xs font-medium text-zinc-500/70">{ad.name}</p>
              <p className="mt-2 text-sm leading-relaxed text-zinc-200/90">{ad.primaryText}</p>
              <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
                <span className="rounded-lg border border-zinc-600/20 bg-zinc-600/10 px-2.5 py-1 text-[11px] font-semibold text-zinc-300">
                  CTA: {ad.cta}
                </span>
                <span className="font-mono text-[10px] text-zinc-600/45">{ad.id}</span>
              </div>
            </div>
          </motion.li>
        ))}
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h2 className="text-lg font-semibold text-zinc-50">Pratinjau struktur adset</h2>
          <p className="mt-1 max-w-2xl text-sm text-zinc-300/55">
