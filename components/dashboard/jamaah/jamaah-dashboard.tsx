...
function paymentBadgeClass(p: PaymentStatus) {
  if (p === "paid") return "border-purple-400/35 bg-purple-500/15 text-purple-100";
  if (p === "partial") return "border-violet-400/35 bg-violet-500/12 text-violet-100";
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        >
          <div>
            ...
            <h1 className="mt-2 text-3xl font-semibold tracking-tight text-purple-50 sm:text-4xl">Jamaah</h1>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Link
              ...
              className="inline-flex items-center gap-2 rounded-xl border border-violet-400/35 bg-gradient-to-r from-purple-500/30 to-violet-500/20 px-4 py-2.5 text-sm font-bold text-purple-50 shadow-lg shadow-purple-950/25 transition hover:border-violet-300/50"
            >
            ...
            <Link
              href="/"
              className="inline-flex w-fit rounded-xl border border-white/10 bg-white/[0.04] px-4 py-2.5 text-sm text-purple-200/80 transition hover:border-purple-400/25 hover:bg-purple-500/10"
            >
              ← Hub
            </Link>
          </div>
        ...
        <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-end">
          <label className="block min-w-[200px] flex-1 sm:max-w-md">
            <span className="text-xs font-semibold uppercase tracking-wider text-purple-300/60">Cari</span>
            <input
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              placeholder="Nama, ID, paket, atau cabang…"
              className="mt-1.5 w-full rounded-xl border border-purple-500/15 bg-purple-950/40 px-3 py-2.5 text-sm text-purple-50 placeholder:text-purple-500/35 focus:border-purple-400/40 focus:outline-none focus:ring-2 focus:ring-purple-500/20"
            />
          </label>
          <label className="block w-full sm:w-44">
            <span className="text-xs font-semibold uppercase tracking-wider text-purple-300/60">Pembayaran</span>
            <select
              value={paymentFilter}
              onChange={(e) => setPaymentFilter(e.target.value as typeof paymentFilter)}
              className="mt-1.5 w-full cursor-pointer rounded-xl border border-purple-500/15 bg-purple-950/40 px-3 py-2.5 text-sm text-purple-50 focus:border-purple-400/40 focus:outline-none focus:ring-2 focus:ring-purple-500/20"
            >
          ...
          <label className="block w-full sm:w-44">
            <span className="text-xs font-semibold uppercase tracking-wider text-purple-300/60">Dokumen</span>
            <select
              value={docFilter}
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-5">
          <div className="lg:col-span-3">
            <div className="glass-panel overflow-hidden rounded-2xl">
              ...
              <ul className="max-h-[min(70vh,640px)] divide-y divide-purple-500/10 overflow-y-auto [scrollbar-color:rgba(52,211,153,0.25)_transparent] [scrollbar-width:thin]">
                {filtered.map((j, i) => {
                    ...
                    <motion.li key={j.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.02 }}>
                      <button
                        ...
                        className={`flex w-full flex-col gap-2 px-4 py-3 text-left transition sm:flex-row sm:items-center sm:justify-between sm:px-5 ${
                          active ? "bg-purple-500/10 ring-1 ring-inset ring-purple-400/25" : "hover:bg-purple-950/40"
                        }`}
                      >
                        <div className="min-w-0">
                          <p className="font-medium text-purple-50">{j.name}</p>
                          <p className="font-mono text-xs text-purple-400/55">{j.id}</p>
                          <p className="mt-1 truncate text-xs text-purple-200/50">
                          ...
                          <span
                            className={`rounded-md border px-2 py-0.5 text-[10px] font-semibold ${
                              isDocComplete(j.documents)
                                ...
                                : "border-white/10 bg-white/[0.04] text-purple-300/70"
                            }`}
                          >
              ...
              {filtered.length === 0 ? (
                <p className="px-5 py-10 text-center text-sm text-purple-500/50">Tidak ada jamaah yang cocok dengan filter.</p>
              ) : null}
            </div>
          </div>
          ...
          <div className="lg:col-span-2">
            <AnimatePresence mode="wait">
              {selected ? (
                ...
                >
                  <p className="text-xs font-semibold uppercase tracking-wider text-purple-400/70">Checklist dokumen</p>
                  <h2 className="mt-1 text-lg font-semibold text-purple-50">{selected.name}</h2>
                  <p className="font-mono text-xs text-purple-400/55">{selected.id}</p>
                  <p className="mt-2 text-sm text-purple-200/55">{selected.packageName}</p>
                  <p className="text-xs text-purple-500/50">
                    {selected.branch} · {selected.batch}
                  </p>

                  <div className="mt-5 rounded-xl border border-purple-500/10 bg-purple-950/30 p-3">
                    <p className="text-xs font-medium text-purple-300/60">Status pembayaran</p>
                  ...
                  <ul className="mt-6 space-y-3">
                    {(
                      ...
                      <li
                        key={row.key}
                        className="flex items-start gap-3 rounded-xl border border-purple-500/10 bg-purple-950/25 px-3 py-3"
                      >
                        <input
                          type="checkbox"
                          checked={selected.documents[row.key]}
                          onChange={(e) => setDoc(selected.id, row.key, e.target.checked)}
                          className="mt-0.5 h-4 w-4 rounded border-purple-500/40 bg-purple-950 text-purple-500 focus:ring-purple-500/40"
                        />
                        <div>
                          <p className="font-medium text-purple-50">{row.label}</p>
                          <p className="text-xs text-purple-500/55">{row.hint}</p>
                        </div>
                      </li>
                    ))}
                  </ul>
                  <p className="mt-4 text-center text-[11px] text-purple-500/45">
                ...
                >
                  <p className="text-sm text-purple-200/55">Sesuaikan filter atau pencarian untuk menampilkan jamaah.</p>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
