...
function passportBadgeClass(s: PassportStatus) {
  if (s === "verified") return "border-zinc-500/35 bg-zinc-600/12 text-zinc-200";
  if (s === "uploaded") return "border-sky-400/35 bg-sky-500/12 text-sky-100";
  if (s === "renewal_needed") return "border-zinc-500/35 bg-zinc-600/12 text-zinc-200";
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        >
          <div>
            ...
            <h1 className="mt-2 text-3xl font-semibold tracking-tight text-zinc-50 sm:text-4xl">Dokumen &amp; Visa</h1>
          ...
          <div className="lg:col-span-5">
            <div className="glass-panel overflow-hidden rounded-2xl">
              <div className="border-b border-zinc-600/10 px-4 py-3 sm:px-5">
                <p className="text-sm font-medium text-zinc-200">Daftar jamaah</p>
                <p className="text-xs text-zinc-600/50">{dokumenRows.length} entri</p>
              </div>
                  ...
                  return (
                    <motion.li key={row.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }}>
                      ...
                      >
                        ...
                        <p className="font-mono text-xs text-zinc-500/55">{row.bookingRef}</p>
                        <div className="flex flex-wrap gap-2">
                          ...
                          <span className="rounded-md border border-zinc-500/25 bg-zinc-700/10 px-2 py-0.5 text-[10px] font-semibold text-teal-100">
            ...
            {selected ? (
              ...
              >
                <section className="glass-panel rounded-2xl p-5 sm:p-6">
                  <p className="text-xs font-semibold uppercase tracking-wider text-zinc-400/60">Status dokumen</p>
                  <h2 className="mt-1 text-lg font-semibold text-zinc-50">{selected.name}</h2>
                  <p className="text-xs text-zinc-600/55">{selected.packageName}</p>
                  <div className="mt-5 grid gap-4 sm:grid-cols-2">
                    <div className="rounded-xl border border-zinc-600/10 bg-zinc-950/30 p-4">
                      <p className="text-[10px] font-bold uppercase tracking-wide text-zinc-600/50">Paspor</p>
                      ...
                      {selected.passportNote ? (
                        <p className="mt-2 text-xs text-zinc-300/55">{selected.passportNote}</p>
                      ) : null}
                    </div>
                    <div className="rounded-xl border border-zinc-600/10 bg-zinc-950/30 p-4">
                      <p className="text-[10px] font-bold uppercase tracking-wide text-zinc-600/50">Visa</p>
                      <p className="mt-2 text-sm text-zinc-200/90">Progres pengajuan e-visa</p>
                      <p className="mt-1 text-xs text-zinc-600/50">Pembaruan: {selected.lastUpdate}</p>
                    </div>
                  </div>
                </section>
