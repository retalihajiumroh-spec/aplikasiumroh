...
function passportBadgeClass(s: PassportStatus) {
  if (s === "verified") return "border-purple-400/35 bg-purple-500/12 text-purple-100";
  if (s === "uploaded") return "border-sky-400/35 bg-sky-500/12 text-sky-100";
  if (s === "renewal_needed") return "border-violet-400/35 bg-violet-500/12 text-violet-100";
  ...
  return (
    <div className="relative min-h-dvh overflow-x-hidden pb-20 pt-6 sm:pt-10">
      <div
        ...
        style={{
          backgroundImage:
            "radial-gradient(circle at 20% 0%, rgba(156, 77, 204, 0.12), transparent 40%), radial-gradient(circle at 90% 60%, rgba(128, 0, 0, 0.1), transparent 42%)",
        }}
      />
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        >
          <div>
            ...
            <h1 className="mt-2 text-3xl font-semibold tracking-tight text-purple-50 sm:text-4xl">Dokumen &amp; Visa</h1>
          ...
          <div className="lg:col-span-5">
            <div className="glass-panel overflow-hidden rounded-2xl">
              <div className="border-b border-purple-500/10 px-4 py-3 sm:px-5">
                <p className="text-sm font-medium text-purple-100">Daftar jamaah</p>
                <p className="text-xs text-purple-500/50">{dokumenRows.length} entri</p>
              </div>
              <ul className="max-h-[min(68vh,640px)] divide-y divide-purple-500/10 overflow-y-auto [scrollbar-color:rgba(52,211,153,0.25)_transparent] [scrollbar-width:thin]">
                  ...
                  return (
                    <motion.li key={row.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }}>
                      ...
                      >
                        ...
                        <p className="font-mono text-xs text-purple-400/55">{row.bookingRef}</p>
                        <div className="flex flex-wrap gap-2">
                          ...
                          <span className="rounded-md border border-violet-400/25 bg-violet-700/10 px-2 py-0.5 text-[10px] font-semibold text-teal-100">
            ...
            {selected ? (
              ...
              >
                <section className="glass-panel rounded-2xl p-5 sm:p-6">
                  <p className="text-xs font-semibold uppercase tracking-wider text-purple-300/60">Status dokumen</p>
                  <h2 className="mt-1 text-lg font-semibold text-purple-50">{selected.name}</h2>
                  <p className="text-xs text-purple-500/55">{selected.packageName}</p>
                  <div className="mt-5 grid gap-4 sm:grid-cols-2">
                    <div className="rounded-xl border border-purple-500/10 bg-purple-950/30 p-4">
                      <p className="text-[10px] font-bold uppercase tracking-wide text-purple-500/50">Paspor</p>
                      ...
                      {selected.passportNote ? (
                        <p className="mt-2 text-xs text-purple-200/55">{selected.passportNote}</p>
                      ) : null}
                    </div>
                    <div className="rounded-xl border border-purple-500/10 bg-purple-950/30 p-4">
                      <p className="text-[10px] font-bold uppercase tracking-wide text-purple-500/50">Visa</p>
                      <p className="mt-2 text-sm text-purple-100/90">Progres pengajuan e-visa</p>
                      <p className="mt-1 text-xs text-purple-500/50">Pembaruan: {selected.lastUpdate}</p>
                    </div>
                  </div>
                </section>
