...
export function OverviewCharts({ data }: { data: OverviewMonthlyPoint[] }) {
  return (
    <div className="grid gap-6 lg:grid-cols-2">
      ...
      >
        <h2 className="text-lg font-semibold text-emerald-50">Revenue trend</h2>
        <p className="mt-1 text-sm text-emerald-200/55">Gross package bulanan (IDR).</p>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={data} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
              ...
              <YAxis
                ...
                width={52}
              />
              <Tooltip content={<RevenueTooltip />} cursor={{ stroke: "rgba(52, 211, 153, 0.2)" }} />
              <Area
                type="monotone"
                dataKey="revenueIdr"
                stroke="#34d399"
      ...
      <motion.section
        ...
        className="glass-panel rounded-2xl p-5 sm:p-6"
      >
        <h2 className="text-lg font-semibold text-emerald-50">Leads trend</h2>
        <p className="mt-1 text-sm text-emerald-200/55">Jumlah lead baru tercatat per bulan.</p>
        <div className="mt-6 h-[260px] w-full min-w-0 sm:h-[300px]">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
              ...
              <Tooltip content={<LeadsTooltip />} cursor={{ fill: "rgba(52, 211, 153, 0.06)" }} />
              <Bar dataKey="leads" name="Leads" fill="rgba(45, 212, 191, 0.65)" radius={[6, 6, 0, 0]} maxBarSize={36} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </motion.section>
