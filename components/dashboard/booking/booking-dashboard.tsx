...
function statusClass(s: BookingPaymentStatus) {
  if (s === "paid") return "border-purple-400/35 bg-purple-500/15 text-purple-100";
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        >
          <div>
            ...
            <h1 className="mt-2 text-3xl font-semibold tracking-tight text-purple-50 sm:text-4xl">Booking &amp; Payment</h1>
          ...
          <div className="lg:col-span-5">
            <div className="glass-panel overflow-hidden rounded-2xl">
              <div className="border-b border-purple-500/10 px-4 py-3 sm:px-5">
                <p className="text-sm font-medium text-purple-100">Booking &amp; pembayaran</p>
                <p className="text-xs text-purple-500/50">{bookingPayments.length} transaksi</p>
              </div>
                    ...
                    <motion.li key={row.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }}>
                      ...
                      >
                        <div className="flex flex-wrap items-center justify-between gap-2">
                          <span className="font-mono text-xs font-semibold text-purple-200">{row.bookingRef}</span>
                          <span className={`rounded-md border px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide ${statusClass(row.status)}`}>
                            {statusLabel(row.status)}
                          </span>
                        </div>
                        <p className="font-medium text-purple-50">{row.customerName}</p>
                        <p className="line-clamp-2 text-xs text-purple-200/55">{row.packageName}</p>
                        <div className="flex flex-wrap items-center justify-between gap-2 text-xs">
                          <span className="font-mono text-purple-300/80">{formatIdrCompact(row.amountIdr)}</span>
                          <span className="text-purple-500/45">{row.invoiceNumber}</span>
                        </div>
                      </button>
                    </motion.li>
                  );
          ...
          <div className="lg:col-span-7">
            {selected ? (
              <div className="space-y-4">
                <p className="text-xs font-semibold uppercase tracking-wider text-purple-300/60">Payment invoice preview</p>
