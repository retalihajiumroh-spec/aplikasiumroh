...
export function CarouselPreview({ slides }: { slides: CarouselSlide[] }) {
  return (
    <div>
      <p className="text-xs font-semibold uppercase tracking-wider text-emerald-300/60">Carousel preview</p>
      <p className="mt-1 text-xs text-emerald-200/45">Pratinjau rasio 4:5 seperti feed Instagram.</p>
      <div className="mt-4 flex gap-3 overflow-x-auto pb-2 pt-1 [-ms-overflow-style:none] [scrollbar-width:thin] [&::-webkit-scrollbar]:h-1.5 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-emerald-500/25">
        {slides.map((slide, i) => (
          <motion.div
            key={slide.id}
            ...
            className="relative h-[220px] w-[176px] shrink-0 overflow-hidden rounded-2xl border border-emerald-500/15 shadow-lg shadow-black/30"
          >
            ...
            <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/70 to-transparent p-3 pt-10">
              ...
              <p className="mt-0.5 text-[11px] text-emerald-100/75">{slide.subtitle}</p>
            </div>
