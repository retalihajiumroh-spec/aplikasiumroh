...
function ImpactDot({ impact }: { impact: EngagementRecommendation["impact"] }) {
  ...
  return (
    <span
      className={
        strong
          ? "rounded-full bg-emerald-400/90 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wide text-emerald-950"
          : "rounded-full border border-emerald-500/25 bg-emerald-950/50 px-2 py-0.5 text-[9px] font-semibold uppercase tracking-wide text-emerald-300/70"
      }
    >
...
export function EngagementOptimizationSection({
  ...
  return (
    ...
    >
      <h2 className="text-lg font-semibold text-emerald-50">Optimasi engagement</h2>
      ...
      <div className="mt-8 rounded-2xl border border-emerald-500/10 bg-emerald-950/25 p-5 sm:p-6">
          ...
          {recommendations.map((rec, i) => (
            ...
            >
              <div className="min-w-0 flex-1">
                <p className="font-medium text-emerald-50">{rec.title}</p>
                <p className="mt-1 text-sm leading-relaxed text-emerald-200/60">{rec.detail}</p>
              </div>
