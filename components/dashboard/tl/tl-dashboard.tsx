...
function rankAccent(rank: number) {
  if (rank === 1) return "from-amber-400/25 via-amber-500/10 to-emerald-950/30 border-amber-400/30";
  if (rank === 2) return "from-slate-300/20 via-slate-400/8 to-emerald-950/30 border-slate-400/25";
  if (rank === 3) return "from-orange-400/18 via-orange-600/8 to-emerald-950/30 border-orange-400/25";
  return "from-emerald-500/8 to-emerald-950/25 border-emerald-500/12";
}
      ...
      <div className="flex items-start gap-3">
        <div
          className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border font-mono text-lg font-bold ${
            person.rank <= 3 ? "border-white/15 bg-white/10 text-emerald-50" : "border-emerald-500/20 bg-emerald-950/40 text-emerald-200/90"
          }`}
          aria-hidden
        >
          #{person.rank}
        ...
        <div className="min-w-0 flex-1">
          <h3 className="font-semibold leading-snug text-emerald-50">{person.name}</h3>
          <p className="mt-1 text-xs text-emerald-200/50">{person.branch}</p>
          <p className="mt-2 text-xs leading-relaxed text-emerald-200/65">{person.highlight}</p>
        </div>
        <div className="shrink-0 text-right">
          <p className="font-mono text-xl font-semibold tabular-nums text-emerald-100">{person.score}</p>
          <p className="text-[10px] font-medium uppercase tracking-wider text-emerald-400/55">Skor</p>
        </div>
      </div>
    </motion.article>
  );
  ...
  return (
    <div className="relative min-h-dvh overflow-x-hidden pb-20 pt-6 sm:pt-10">
      <div
        ...
        style={{
          backgroundImage:
            "radial-gradient(circle at 18% 0%, rgba(52, 211, 153, 0.12), transparent 42%), radial-gradient(circle at 88% 45%, rgba(251, 191, 36, 0.06), transparent 40%)",
        }}
      />
      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        ...
        >
          <div>
            ...
            <h1 className="mt-2 text-3xl font-semibold tracking-tight text-emerald-50 sm:text-4xl">TL &amp; Muthowif</h1>
        ...
        <div className="mt-10 grid gap-8 lg:grid-cols-2">
          <section>
            <div className="mb-4 flex items-end justify-between gap-2 border-b border-emerald-500/10 pb-3">
              <div>
                <h2 className="text-lg font-semibold text-emerald-50">Ranking Tour Leader</h2>
                <p className="mt-0.5 text-xs text-emerald-200/50">Periode rolling 90 hari · skor komposit internal.</p>
              </div>
              <span className="rounded-lg border border-emerald-500/15 bg-emerald-950/40 px-2 py-1 font-mono text-[10px] text-emerald-300/80">Top 5</span>
            </div>
          ...
          <section>
            <div className="mb-4 flex items-end justify-between gap-2 border-b border-emerald-500/10 pb-3">
              <div>
                <h2 className="text-lg font-semibold text-emerald-50">Ranking Muthowif</h2>
                <p className="mt-0.5 text-xs text-emerald-200/50">Materi manasik, kehadiran, &amp; umpan balik jamaah.</p>
              </div>
              <span className="rounded-lg border border-emerald-500/15 bg-emerald-950/40 px-2 py-1 font-mono text-[10px] text-emerald-300/80">Top 5</span>
            </div>
            <div className="space-y-3">
        ...
        <section className="mt-12">
          <div className="mb-4 border-b border-emerald-500/10 pb-3">
            <h2 className="text-lg font-semibold text-emerald-50">Penugasan Team Leader</h2>
          ...
          <ul className="space-y-4">
            {rows.map((row, i) => (
              <motion.li
                ...
                className="glass-panel flex flex-col gap-4 rounded-2xl border border-emerald-500/10 p-4 sm:flex-row sm:items-center sm:justify-between sm:p-5"
              >
                <div className="min-w-0">
                  <p className="font-mono text-xs font-semibold text-emerald-300/75">{row.code}</p>
                  <h3 className="mt-0.5 text-base font-semibold text-emerald-50">{row.packageName}</h3>
                  <p className="mt-1 text-sm text-emerald-200/50">{row.departureLabel}</p>
                </div>
                <div className="flex shrink-0 flex-col items-stretch gap-2 sm:min-w-[200px] sm:items-end">
                  {row.tlName ? (
                    <div className="rounded-xl border border-emerald-400/25 bg-emerald-500/10 px-3 py-2 text-right">
                      <p className="text-[10px] font-bold uppercase tracking-wide text-emerald-400/70">TL aktif</p>
